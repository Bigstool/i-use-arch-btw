...
| grub              | GNU GRand Unified Bootloader (2)                             | Add `--removable` option when running `grub-install` to allow for dual booting with Windows or booting on another machine. Not necessary if using systemd-boot. |                |            |                                        |
| efibootmgr        | Linux user-space application to modify the EFI Boot Manager  | Not necessary if using systemd-boot.                         |                |            |                                        |
...
| bluez             | Daemons for the bluetooth protocol stack                     | `sudo systemctl enable bluetooth`                            |                | x          |                                        |
| ufw               | Uncomplicated and easy to use CLI tool for managing a netfilter firewall | https://barn.bigstool.com/post/ufw/                          |                | x          |                                        |
| openssh           | SSH protocol implementation for remotelogin, command execution and file transfer | `sudo systemctl enable sshd`                                 |                | x          |                                        |
...
| Name               | Description                       | Comment                    | Unverified (x) | Daemon (x) | 3rd Party Repo URL |
| ------------------ | --------------------------------- | -------------------------- | -------------- | ---------- | ------------------ |
| nvidia-open-lts    | NVIDIA drivers for linux-lts      | For the `linux-lts` kernel |                |            |                    |
| nvidia-open        | NVIDIA kernel modules             | For the `linux` kernel     |                |            |                    |
